...
| Offset | Field | Type | Meaning |
| :--- | :--- | :--- | :--- |
| `[0..4)` | `kind` | `u32` | The arena the section holds, as the row number of Appendix A: 2 neurons, 3 synapse blocks, 37 plastic deltas; 1, 38 and 39 named and Specified. |
...
Implemented rule: `execute` performs the opcode with `i128` checked arithmetic; `OP_MUL_Q16` is $(a \times b) \gg 16$ and `OP_DIV_Q16` is $(a \ll 16) / b$; on success the result is stored and the flags cleared, on any error the result is zero and one flag names it; `OP_REM` wraps after the zero check, since the remainder of `i128::MIN` by $-1$ is exactly 0 and fits ([ADR-0028](adr/0028-edge-behaviour-audit.md)). Seven tests, including `i128::MIN / -1`, that remainder, and the pre-shift overflow of `OP_DIV_Q16`.
...
| V-2 Vertical | Does the source tree still contain what this document says it contains? | [`@descent-vtt/spec-guard`](https://www.npmjs.com/package/@descent-vtt/spec-guard) executing the `@assert-*` directives in this file, the README, `CONTRIBUTING.md`, `CLAUDE.md` and the live briefs | CI, blocking |
