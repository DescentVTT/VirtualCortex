...
| `cortex-reasoning` | `SymbolicRuleNode`, `TermNode` | 64 B, 64 B | yes | yes | yes | `evaluate`, `resolve`, `apply_resolution`, `unify`, `resolve_first_order`, `reduce` (categorial reduction), `head`, `lgg`, `absorb`, `identify`, `intra_construct`, `resolve_definite` (induction) |
...
| Compression progress as reward (Rissanen; Hinton–Zemel; Schmidhuber) | `cortex-affect` · `update_valence`; `cortex-neuromod` · `reward`; the runtime's `invent` | A clause store's description length is the free energy; an invention's drop is the valence; a quarter of it, clamped, is the reward-prediction error under which the modulator consolidates pending traces. | Implemented ([ADR-0043](adr/0043-discovery-path.md)); its effect on a later behaviour is H-11 |
...
- [ ] **H-11 (compression progress as a reward; first-fit matching).** That a description-length drop ([ADR-0043](adr/0043-discovery-path.md)) consolidating the eligibility traces pending at that moment, on a network that computes anything, biases what a later behaviour reads toward the invention: Schmidhuber's theory is about curiosity and aesthetics, and what the tree holds is a mechanism whose exit test moves one weight by a pinned amount. And that the first-fit matching of the operators of [ADR-0041](adr/0041-induction-on-the-term-arena.md), which pairs each literal with the first free partner that unifies and never backtracks, finds the pairings a clause store of a later round needs (the greedy gap of H-10, restated for induction). The protocol: a clause store and a behaviour that reads it, the rate at which rewarded inventions are read back against unrewarded ones; a corpus of clause pairs, the pairing rate against a backtracking oracle.
