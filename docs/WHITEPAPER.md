...
<!-- @assert-count target="runtime/cortex-runtime/src/episode.rs" symbol="fn discover" min="1" reason="ADR-0050: the search, the reward and the tag run in one call on the executor's own train" -->
...
| STDP | `cortex-core` · `last_soma_spike_tick`, `SynapseBlock::{last_spike_tick, step_stdp}`, `Polarity` | Nearest-neighbour pairs at the presynaptic spike $t$ on the weight's magnitude under the block's polarity, its presynaptic unit's. Excitatory: $+A_+ (1 - 2^{-11})^{q - p}$ when the target's last spike $q$ follows the previous presynaptic spike $p$; then $-A_- (1 - 2^{-11})^{t - q}$ when $q$ precedes $t$; $A_+ = 0.0100$, $A_- = 0.0105$. Inhibitory (Vogels et al. 2011): $-\alpha$ at every presynaptic spike, $+A_+ (1 - 2^{-11})^{\lvert\Delta t\rvert}$ for each pairing whichever way round; $\alpha = 2 \rho_0 \tau A_+ = 0.0020$ at $\rho_0$ = 5 Hz. Q1.15 saturating, into the eligibility trace since [ADR-0032](adr/0032-three-factor-plasticity.md). | Implemented ([ADR-0022](adr/0022-synapse-fan-out-and-stdp.md), [ADR-0049](adr/0049-dale-principle-in-plasticity.md)); the $m = 1$ limit of the row below; the target rate a constant until a registry entry |
