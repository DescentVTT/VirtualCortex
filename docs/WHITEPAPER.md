...
| Synaptic scaling per unit (Turrigiano) | `cortex-core` · `[52..54)` of `DendriticSuperNeuron` | A gain per unit, lowered by a step at each of its spikes and relaxing toward its ceiling between spikes by $(1 - 2^{-k})^{\Delta t}$ at the next event, applied by the turn beside the population gain; a silent unit's gain rises, a busy one's falls, with no sweep. | Specified ([ADR-0036](adr/0036-criticality-control.md)); what it answers is read in [ADR-0053](adr/0053-the-waking-day-and-the-target-period.md)'s day: under a stationary drive at the gain the controller holds, the depression-dominant pair rule drains the reference network's excitatory arena to nothing within fifteen windows at 1 024 units and by a sixth in eight at 256 |
