...
<!-- @assert-count target="crates" symbol="deny(clippy::arithmetic_side_effects)" glob="*.rs" min="15" reason="ADR-0029: the arithmetic lint is denied in every crate that passes it; the count only grows" -->
...
Implemented rule: `execute` performs the opcode with `i128` checked arithmetic; `OP_MUL_Q16` is $(a \times b) \gg 16$ and `OP_DIV_Q16` is $(a \ll 16) / b$; on success the result is stored and the flags cleared, on any error the result is zero and one flag names it; `OP_REM` wraps after the zero check, since the remainder of `i128::MIN` by $-1$ is exactly 0 and fits ([ADR-0028](adr/0028-edge-behaviour-audit.md)). Eight tests, including `i128::MIN / -1`, that remainder, the pre-shift overflow of `OP_DIV_Q16`, and every opcode against a checked reference over the lattice.
