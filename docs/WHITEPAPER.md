...
| F-42 | A timeout of the weekly sweep can be a slow pass starved by the hang running beside it, and `timeout.txt` does not tell the two apart. `image.rs:718:24: delete ! in Image::decode` timed out in all four complete sweeps (`35284722491`, `35374662481`, `35444829805`, `35459078284`), always in `runtime-0`; its per-mutant log ends inside `criticality`, a binary that decodes nothing and that the shard's caught mutants finish in 8 to 95 s, and the tool's `debug.log` shows the other of the shard's two slots (`--jobs 2`, [ADR-0058](adr/0058-the-weekly-sweep-and-its-timeouts.md)) running `Injector::push`'s spinning hang from 3 207 s to 4 348 s of the job while this mutant ran from 3 197 s to 4 341 s. Alone, the mutant is caught; in the sweep dispatched on brief 028's branch (`35485126396`) it was caught and the `clauses` field's mutant of the same function took its place in `timeout.txt`, its log ending in `reference` beside `Injector::push`'s hang: the class recurs, not the mutant. | `.github/workflows/ci.yml` | **Open** ([ADR-0062](adr/0062-the-first-complete-sweeps-list.md)): the triage rule's fifth kind, not a loop of the code; V-6's "counted as caught" covers a slow pass as it covers a hang, so nothing is hidden, and the sweep is unchanged this round (its `--jobs`, bounds, shards and completeness check are ADR-0058's, which brief 028 did not touch). A next change to the sweep runs the runtime shards at `--jobs 1`, or reads `debug.log` for the neighbour before reading a timeout as a hang; until then a timeout whose log ends in a binary that does not test the mutant is read as this class. |
