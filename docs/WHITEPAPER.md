...
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="_exhaustive" min="23" reason="ADR-0066, ADR-0069, ADR-0070, ADR-0072 and ADR-0074: the ten full runs, the two calibrations, the six addressed runs, the settling, the run behind the lead-in, the composition, the ladder and the candidate stimuli are weekly" -->
...
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="const LEAD_IN_WINDOWS: u64 = 9;" min="1" reason="ADR-0070: the lead-in derived from the settling table, nine windows, committed before the run behind it" -->
...
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="fn volleyed" min="1" reason="ADR-0072: the class rule that splits a term by the readout spike that paired it" -->
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="const GAIN_PICKED_1024: Option<u32> = None;" min="1" reason="ADR-0072: the ladder picked no rung, and the gate holds the constant to the pick over the pinned tables" -->
...
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="const SHAPE_PICKED_1024: Option<Shape> = None;" min="1" reason="ADR-0074: no candidate fires every unit once, and the gate holds the constant to the pick over the pinned tables" -->
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="fn fires_once" min="1" reason="ADR-0074: the measure that picks a candidate, the volley clause and the after clause" -->
<!-- @assert-count target="runtime/cortex-runtime/tests/instrument.rs" symbol="fn offset" min="1" reason="ADR-0074: the offset the criterion needs, by a scan on the recorded counts" -->
...
The whole-domain line runs as one command here and as three shards in the weekly job, taken from the binaries `--list` names at run time and each checked against what the list counted ([ADR-0073](adr/0073-the-whole-domain-tests-sharded.md)); each shard reports what its binaries took ([ADR-0071](adr/0071-the-exhaustive-job-times-its-own-binaries.md), F-45).
