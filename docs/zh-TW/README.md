...
| [§11 Risks and technical debt](../WHITEPAPER.md#11-risks-and-technical-debt) | 編號 finding F-1 至 F-36、假設 H-1 至 H-11（H-3 是「這些規則是否構成經驗」、H-5 是「再表徵是否算原創綜合」、H-6 是「標了玩笑的話是否真的好笑」，倉庫裡沒有任何測試能判定，文件因此不主張；H-8 是分支比估計器在參考規模是否讀到分支比（在 256 與 1 024 單元上已量測：單一視窗的斜率不是 σ 的讀數，控制器在飽和上限處轉向；在第二個先驗——稀疏隨機網路——上再量測後，記錄不加第二行：粗 bin 的雜訊來自 bin 的長度，細 bin 的斜率讀的是被驅動族群的自相關而非分支比，ADR-0047；附錄 A 的規模仍為 Target）、H-9 是睡眠中的重播在參考規模是否鞏固任何東西（同樣已量測：局部模式在一夜後可由半數提示補全，隨機模式不能；由脈衝序列自行標記的模式也已量測：經驗的模式能補全六個中的五個，網路自身最密的巧合不能，ADR-0048）、H-10 是貪婪的移位歸約器是否涵蓋模板所需的構式、H-11 是壓縮進展當作獎賞所固化的痕跡是否被之後的行為讀到（符號的一半已裁定：發明出的謂詞在之後的證明中被讀回；突觸的一半也已在此規模裁定：有獎賞的發明有了一個由獎賞前的漣漪標記出來的單元模式，一夜鞏固後提示一半能點燃其餘，關聯表由呼叫者持有，ADR-0048），以及先到先配的文字配對是否找得到子句庫需要的配法，各附量測協定）、開放問題。 |
