...
- Good: the engine has one thing it can change about itself, and every such change is recorded, gated four times, tested in a fork against the live baseline, committed only when the fork behaved identically, persisted, replayed on load and refused by the loader when forged. The veto gate is consulted by id and cannot be amended. A run is still `(image, seed, trace)`.
...
- `crates/cortex-executive/src/lib.rs`: `PolicyAmendment`, the constants, `REGISTRY`, `spec_of`, `bounds_reason`; fourteen unit tests and the two property tests.
...
- `runtime/cortex-runtime/src/{executor,image,trial}.rs`: `Policy`, `AmendError`, `Executor::{policy, amendments, amendment_room, propose, admit, record_trial, commit, sweep_by_policy}`, the section in `Image::{encode, decode}`, `trial::run`; `runtime/cortex-runtime/tests/amendment.rs`: the closed loop end to end, the rejection for no gain, the stale commit, persistence and the loader's refusals.
