...
- **What is measured and what is not.** Splitting by area was half the answer on its own: the tool scopes its baseline to the packages that hold the area's mutants, so `crates` fell from a 434-second baseline to three seconds and from an implied 2 172-second bound to 39. The six runtime shards were then measured in run [35284722491](https://github.com/DescentVTT/VirtualCortex/actions/runs/35284722491), and **the whole tree swept for the first time: 3 396 of 3 396 mutants, every job green, the slowest in 1 h 58 m**, so no shard comes near the three-hour rule and `n = 6` stands:

  | Job | Suite on its runner | Bound | Tested | Caught | Missed | Timeout | Unviable | Job |
  | :--- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
  | `crates` | 7 s | 51 s | 2 417 / 2 417 | 2 311 | 13 | 16 | 77 | 27 m |
  | `runtime-0` | 386 s | 1 188 s | 164 / 164 | 147 | 1 | 5 | 11 | 1 h 48 m |
  | `runtime-1` | 379 s | 1 167 s | 163 / 163 | 150 | 0 | 5 | 8 | 1 h 45 m |
  | `runtime-2` | 464 s | 1 422 s | 163 / 163 | 151 | 1 | 2 | 9 | 1 h 26 m |
  | `runtime-3` | 442 s | 1 356 s | 163 / 163 | 151 | 2 | 1 | 9 | 1 h 21 m |
  | `runtime-4` | 371 s | 1 143 s | 163 / 163 | 150 | 2 | 2 | 9 | 1 h 58 m |
  | `runtime-5` | 216 s | 678 s | 163 / 163 | 151 | 2 | 4 | 6 | 1 h 10 m |
  | **Whole tree** | | | **3 396 / 3 396** | **3 211** | **21** | **35** | **129** | |

  The runtime's own suite ran between 216 and 464 seconds on six runners doing identical work, a spread of more than two to one. That is the sibling repositories' lesson ("runners vary by a fifth or more on identical work") at twice their figure, and it is what a bound written into this file could not have absorbed: derived on each runner, it tracked its runner. What the whole sweep now costs is not predicted here: the run that follows this change measures it, and the number enters Appendix B and this ADR's consequences when it exists. No figure in this ADR is a timing claim about the engine; every one is the cost of running its tests ([ADR-0010](0010-measured-or-target.md) governs the engine's figures, not CI's wall clock).
- **What the first complete sweep found.** Twenty-one survivors over the whole tree: thirteen under `crates/**` and eight under `runtime/**`. They are not this ADR's subject and they block nothing: ADR-0030 makes them the next round's list, and whitepaper §11.1 carries them as one.
