...
- **The benchmark.** `executor/idle_tick/{1,2,4}` in `benches/cortex-bench`: an executor of eight units at rest, one `tick` per iteration, so the four barrier waits, the coordinator's publication of the modulation and the gain and the tally are all it measures. A developer-machine figure is recorded as not admissible in `docs/benchmarks/results/2026-09-12-dancr-win11.md` (ADR-0010): 52 ns on one worker, 570 ns on two, 1.68 µs on four, unpinned; the admissible run is the protocol of `docs/benchmarks/README.md` on the reference platform at its worker count.
