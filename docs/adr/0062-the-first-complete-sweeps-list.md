...
| `Walk::advance` replaced by `Ok(())` (`fraction.rs:151`) | 1 | Directive-class, 3 | Not a loop: a step that does not move the walk, which hung the three callers whose loops asked the walk where it was. `convergent` (`fraction.rs:175`), whose own mutants were all caught, has the same shape and was **rewritten** by the rule, not by the sweep: `for _ in 0..depth`. With the three walks ranges, this mutant is caught by their exact tests. |
...
The in-diff gate of the pull request holds every test and restructuring above (each is in the diff), and reports what it found in the pull request. The weekly sweep dispatched on this round's branch after the last change is recorded here when it has run: its id, its counts beside `35284722491`'s, and each list's difference. Until then this section says so.

<!-- The dispatched run's table is filled in by the round before the pull request merges. -->
