...
| `Walk::advance` replaced by `Ok(())` (`fraction.rs:151`) | 1 | Directive-class, 3 | Not a loop: a step that does not move the walk, which hung the three callers whose loops asked the walk where it was. `convergent` (`fraction.rs:180`), whose own mutants were all caught, has the same shape and was **rewritten** by the rule, not by the sweep: `for _ in 0..depth`. With the three walks ranges, this mutant is caught by their exact tests. |
