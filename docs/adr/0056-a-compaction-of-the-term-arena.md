...
- Neutral: an index into the arena is valid until the next slow-wave onset or the next call, which the inputs' docs say; the discoveries of the last search are cleared at a compaction for that reason, and a caller that keeps an index across a night keeps garbage, or worse, a moved node's slot, since the inputs admit any index below the cursor. A host builds a clause and asserts it before the ticks cross an onset. The cost is three passes over the cursor at a window boundary: at the reference tests' arena (1 024 nodes) nothing; at Appendix A's arena it is the cost of a sweep, once per night, and a Target like every figure at that scale.
