...
- **ADR-0078's lesson.** A mechanism waits for a measured need. Nothing in this decision needs code.
...
- **What the documents now say.** §1.1 keeps its sentence about the tissue and gains one on what is sparse there and what this engine's cost follows. Quality goal 4 holds in the real-time mode. Appendix A's table is not edited: the paragraph under it says the table is the offline mode's plan and states the real-time mode's Target. F-49 is resolved by this ADR, and §11.1's question on the integration model is decided. A new open question in §11.1 carries the real-time mode's reach.
- **The levers on the real-time mode's reach, named and not taken.** Each waits for the round that builds the embodiment loop, where the real-time budget first binds, and for a measurement on the reference platform:
  1. *A sweep without the gate.* When the fraction served is near one, a worker could serve every unit of a contiguous range without the deque and without the gate's schedule, begin and end. In ADR-0097's bench the gate took 9.79 ns and the integration 11.03 ns of a turn (a developer machine's, not admissible). A change to ADR-0023.
  2. *The layout of the integrated fields.* A sweep that reads only the fields `integrate` reads, a field to an array, moves less memory per unit than the 64-byte record and admits the vector units. [ADR-0001](0001-64-byte-pod-records.md) weighed a struct of arrays and kept it "available inside a subsystem where SIMD gathers need it", the 64-byte record staying the interchange unit and the image's layout; quality goal 2 says a unit's state is one cache line. Whether a working layout beside the record fits both is that layout's ADR's question, taken only if a per-turn breakdown on the reference platform shows the integration binds.
  3. *The rest condition*, as ADR-0097 named it: a unit counted at rest inside the one-LSB tail. This shortens $D$ by 1 023 of 2 503 ticks; it leaves ADR-0044's density at 100 per cent and helps a sparse input. A rule change.
...
- **The order.** The learning line resumes from H-18's named next decision ([ADR-0096](0096-the-punished-pair-measured.md)): the reward-prediction error, a schedule of reversals, the operating regime or another size, all in the offline mode.
