...
- **The sharded job dispatched on this change's branch** at `scope=exhaustive`, run [35558922967](https://github.com/DescentVTT/VirtualCortex/actions/runs/35558922967): three shards, all green, **34 m, 16 m and 14 m**, so the wall clock is 34 minutes of the 120 available where one job read 1 h 43 m — a margin of 3.5 times where it was 1.15. Their tables are `learning` 11 tests in 783 s; `cortex-core` 1 in 2 and `reference` 7 in 938; `instrument` 22 in 2 005. The union is **41 tests**, the count the enumeration and the unsharded run both report, and every shard's completeness check held. The 3 728 s of test time against the 6 174 s of run `35548410926` is the same runner spread ADR-0072 measured there (1.35 to 1.67 times), from the other side.
