...
- Format version 11 ([ADR-0032](0032-three-factor-plasticity.md) carries the bump); a version-10 image is refused by the version check, which comes before the tick's (magic, version, checksum, padding, tick), and had `[60..64)` zero, which `ZeroTick` would refuse on its own, and `[40..48)` zero, which would have resumed at tick 0 as before.
