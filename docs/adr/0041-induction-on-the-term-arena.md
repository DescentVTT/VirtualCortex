...
- **`InduceScratch`** mirrors `ParseScratch`: the arena and `free`, the bindings, the trail and `trail_len`, the work stack, a pair table for the generalisation, `next_variable`, `next_invented`. Every rule marks these on entry (`mark`, an `InduceMark`) and, on any error, `restore`s them: the bindings undone, the nodes it appended zeroed, the cursor and the counters reset; on success the bindings it made stay, the trail saying which, and an output is an instance of its inputs through them. The pair is public so that a caller composing several rules (the runtime's `invent`) restores the same way on a failure of its own.
- **The measures.** `size` is a term's node count through the bindings (a shared subterm counts each time it is reached; saturating); `term_hash` is FNV-1a 32 over the pre-order walk (kind, functor or variable number, arity), the same for the same structure wherever it sits; `free_variables` appends the unbound variables of a term, one node per number, to a caller slice. Every walk visits at most `WALK_LIMIT` = $2^{20}$ nodes, a constant of `term.rs` the occurs check of [ADR-0025](0025-term-arena-and-unification.md) now shares (finding F-32): a cyclic arena, which no builder and no unification makes but a caller's arena can hold, is `BoundExceeded`, not a walk that never ends. A clause node whose arity byte is past eight is not a clause: `Malformed` on either side of every operator.
...
- Bad: a term of more than $2^{20}$ nodes is a bound exceeded for the measures, the price of the cycle guard; no store this round considers comes near it.
...
`cortex-reasoning`: a clause holds a head and up to seven literals and refuses eight, a category is not a clause; `size`, `term_hash` and `free_variables` read through the bindings, the hash ignores where a term sits, every bound and malformed case is a result; the generalisation's textbook cases (`f(a, b, a)` with `f(c, b, c)` is `f(X, b, X)`, different pairs are different variables, nested compounds, different functors, a variable against a constant, a term with itself) and its bounds (`PairsFull`, `BoundExceeded` for the stack and for the variable numbers, `ArenaFull` with the arena zeroed above the mark, `Malformed`); each operator on a named example with the identity checked by `resolve_definite`; a variable only a differing literal uses is not an argument; a clause with a corrupted arity malformed on either side; a cyclic arena and a binding cycle through a compound a bound exceeded; the generalisation's variables numbered in slot order; every error variant reached; the exit pair's three hashes pinned; a second invention takes the next id; the resolvent's literal order; a property walk over 3 000 random clause pairs from a three-predicate vocabulary holding the identities for every success (more than a hundred inventions and absorptions, more than thirty identifications) and the restoration of the arena, the bindings and the counters for every failure. `npx spec-guard` asserts `CLAUSE`, `fn intra_construct` and `fn lgg`. The mutation gate on the changed lines ([ADR-0030](0030-verification-governance.md)) passes in CI.
