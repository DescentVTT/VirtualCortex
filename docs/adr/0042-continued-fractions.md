...
- **What the pins say** (computed by an oracle outside the tree before the tests were written; TC-4 keeps the oracle outside). For the target $2718281828459045 / 10^{15}$ within $10^{-12}$, degree 1, bound 3, depth 20 (2 401 tuples less the zero polynomials), exactly one candidate: $a = [3, 1, 0]$, $b = [0, -1, 0]$, the fraction $e = 3 - 1/(4 - 2/(5 - 3/(6 - \dots)))$, at depth 20 with $p = 2916471173788403280463$ and $q = 1072909785605898240000$; its walk fits to depth 31 and the step to 32 overflows. For $2414213562373095 / 10^{15}$, degree 0, bound 2, depth 40 (sixteen tuples: twenty-five less the nine with a zero polynomial), exactly one: $a = [2, 0, 0]$, $b = [1, 0, 0]$, the silver ratio $1 + \sqrt 2$. A tolerance of one half over the same sixteen tuples at depth 8 matches four, in enumeration order. The same target near $e$ at degree 2 and bound 1 matches nothing: $a_0 = 3$ is outside the bound.
- **What is not claimed.** That a match is an identity: the fraction near $e$ is one because the mathematics says so, not because the search found it; a caller that wants more than a conjecture sends the candidate's hash to the broker ([ADR-0043](0043-discovery-path.md), §6.10). That the search finds a fraction that converges slowly: Brouncker's fraction ($a_n = 2$, $b_n = (2n - 1)^2$; with $a_0 = 2$ its limit is $1 + 4/\pi$) overflows the slot at depth 28, its deepest convergent is $1.5 \times 10^{-2}$ off (the review's exact oracle), and the search reports it only under a tolerance that admits many others.
- **The meet-in-the-middle search** (option 4) is Specified: at this bound the direct comparison is 2 304 walks of at most twenty steps, a cost the caller sees between ticks, and no measurement asks for the larger space where the two-sided match pays.
