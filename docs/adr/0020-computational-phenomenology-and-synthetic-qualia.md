...
| 3 Strange loop | The self-model contains a model of itself and is at a fixed point when they agree | `MentalCanvasFrame::reflect(observed_self_hash)`: the fixed point is reached when the self observed now equals the self observed at the previous reflection; `reflection_converged` records it; a fresh frame has observed nothing and is not at it (`reflection_count`) | Implemented |
