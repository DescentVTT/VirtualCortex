...
- [ ] **The settled network (the next free ADR number; `ls docs/adr`)** (`depends-on: ADR-0076`;
  ...
  assumed.
- [ ] **The controller (the same ADR).** At 1 024 units in the instrument's configuration with
...
- [ ] **The three measures, for each candidate in order — the settled network, then the controller.**
...
- [ ] **What the criterion requires, under each candidate.** ADR-0074's three integer readings — the
...
- [ ] **If a candidate passes — the rule's step 4 — ADR-0069's criterion, unchanged**, at 1 024 units
...
- [ ] **If no candidate passes — the rule's step 3 — the closing, written by this round.** The ADR
...
- [ ] **The gate.** At most one test, running no whole run: the first windows of the settled lead-in
...
- [ ] **The evidence.** A weekly dispatched on this round's branch at the scope
  ...
  bound in the ADR.
- [ ] **The documents, in the same pull request.** Whitepaper §11.1's H-12 and its stopping rule (the
