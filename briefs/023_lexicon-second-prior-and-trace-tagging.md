...
<!-- @assert-absence target="runtime/cortex-runtime/src" symbol="pub fn realise" reason="precondition: no realisation of a frame into token ids exists yet; archived with the brief" -->
<!-- @assert-absence target="crates/cortex-hippocampus/src" symbol="pub fn capture" reason="precondition: no rule tags a pattern from a spike train yet; archived with the brief" -->
<!-- @assert-absence target="runtime/cortex-runtime/tests" symbol="fn tag_discovery" glob="*.rs" reason="precondition: nothing binds a rewarded invention to an episode yet; archived with the brief" -->

## Deliverables

- [ ] **The lexicon ADR (the next free number)** (`docs/adr/0046-*.md`, `depends-on: ADR-0040`,
...
- [ ] **The second-prior ADR (the number after it)** (`docs/adr/0047-*.md`, `depends-on:
...
- [ ] **The trace-tagging ADR (the number after that)** (`docs/adr/0048-*.md`, `depends-on:
...
- [ ] **Finding F-35** in whitepaper §11: the Context's item 8, with the disposition (the
...
- [ ] **H-9's open item and H-11's synaptic half dispositioned** in §11.1 with the numbers;
...
- [ ] **The documents.** Whitepaper 4.11.0: the executive summary's sentence on what exists
...
- [ ] **Not adopted, with the reason in the ADR that is closest:** the online capture inside
