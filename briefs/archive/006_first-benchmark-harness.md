...
> **Executed 2026-09-10 in pull request #10.** Narrows finding F-13; writes
> [ADR-0014](../../docs/adr/0014-benchmark-harness.md); adds `benches/cortex-bench`,
> `docs/benchmarks/README.md` and one results file marked not admissible. The report is in the pull
> request and in `CHANGELOG.md`. The body below describes the tree before execution and is not
> maintained, apart from relative links, which gained one `../` so that they still resolve from
> `archive/`.
...
- [x] A new ADR at the next free number (`ls docs/adr`), `status: proposed` in the PR, choosing
      ...
      accordingly.
      ADR-0014: criterion `=0.7.0` (not 0.8.2: MSRV 1.86 exceeds ADR-0009's proposed 1.85);
      divan rejected on the stable-API clause. Committed `accepted` per `docs/adr/README.md`.
- [x] `benches/cortex-bench/` as a workspace member with `publish = false`, depending on the
      ...
      Brief 005 had landed, so the wheel benchmarks use `schedule` and `advance` on the production
      `WorkerWheel`.
- [x] `docs/benchmarks/README.md`: the protocol for an admissible run (reference platform per
...
- [x] One results file from the machine that executes this brief, marked `admissible: no` with
      the reason (not the reference platform), so that the convention is exercised and the number
      exists without being promoted.
- [x] CI: a step that compiles the benchmarks (`cargo bench -p cortex-bench --no-run` or the
      harness's `--test` mode) so they cannot rot; no timing is asserted in CI.
- [x] ADR-0010 confirmation satisfied: a `spec-guard` directive in the whitepaper asserting
      ...
      updated.
- [x] `CHANGELOG.md` entry under Unreleased, naming the new dependency and its version.
