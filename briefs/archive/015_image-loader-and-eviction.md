...
> **Executed 2026-09-10 in pull request #28.** Writes
> [ADR-0024](../../docs/adr/0024-cortex-image-and-clock-sweep.md); the image round-trips,
> milestone M4's exit test passes as `runtime/cortex-runtime/tests/image.rs`, findings F-20 and
> F-23 are resolved, the image format is version 7, and `mmap`, slot reclamation and a hot
> checkpoint stay Specified. The report is in the pull request and in `CHANGELOG.md`. The body
> below describes the tree before execution and is not maintained, apart from relative links,
> which gained one `../` so that they still resolve from `archive/`, and the §6.7 link, whose
> heading lost its "(Specified)" suffix when the loader landed.
...
- Whitepaper [§6.7](../../docs/WHITEPAPER.md#67-scenario-r-7-cold-boot-from-a-cortex-image)
...
- [x] `cortex-connectome`: `SectionEntry` (64 B, asserted), `SectionKind` constants,
...
- [x] `cortex-core`: `PlasticDelta` (16 B, asserted, per the ADR) and `plastic_delta_head: u32`
      (F-20 resolved), the format bump, §5.2.1 table updated.
- [x] `runtime/cortex-runtime`: `Image::write(&arenas, path)` and `Image::open(path)` per the
...
- [x] Whitepaper §5.2.1, §5.2.2 (directory record, the next version), §6.7 (which steps are
