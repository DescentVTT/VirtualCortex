...
> **Executed 2026-09-10 in pull request #19.** Writes
> [ADR-0018](../../docs/adr/0018-membrane-integration.md); R-1 step 5 is Implemented. The
...
- [x] A new ADR at the next free number (`ls docs/adr`), `status: proposed` in the PR: the
      ...
      ADR-0018: per-compartment shift leaks with a one-LSB floor, difference coupling into the soma, hard refractory window dropping inputs, reset −0.25, BAC plateau Implemented (apical ≥ 0.5 at the spike), threshold adaptation Implemented (+0.02 per spike, decay to 1.0), `ticks_since_spike` for the wrap. Committed `accepted` per `docs/adr/README.md`.
- [x] `DendriticSuperNeuron::integrate(&mut self, basal_q16: i32, apical_q16: i32, now_tick: u32) -> bool`
      ...
      In `crates/cortex-core/src/dynamics/membrane.rs`; fifteen `const` parameters, re-exported from the crate root.
- [x] Tests: leak from a displaced potential reaches rest exactly (no stall above rest);
      ...
      Ten tests; the fire tick is bounded to a window derived from the fixed point rather than pinned to one number, since the coupling lag has no closed form.
- [x] Whitepaper §5.2.1 (public API, status), §6.1 step 5 Implemented, §8.8 rows (LIF and BAC),
