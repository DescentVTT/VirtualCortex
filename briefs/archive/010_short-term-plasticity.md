...
> **Executed 2026-09-10 in pull request #20.** Writes
> [ADR-0019](../../docs/adr/0019-short-term-plasticity.md); the §8.8 short-term-plasticity row is
> Implemented. The report is in the pull request and in `CHANGELOG.md`. The body below describes
> the tree before execution and is not maintained, apart from relative links, which gained one
...
- [x] A new ADR at the next free number (`ls docs/adr`), `status: proposed` in the PR: the working
      ...
      ADR-0019: computed in Q16.16, rounded to nearest with a one-LSB floor toward the target; per spike with the elapsed interval; constants in `cortex-core`; the fields stay Q0.8, so no format bump. Committed `accepted` per `docs/adr/README.md`.
- [x] `DendriticSuperNeuron::step_stp(&mut self, ...)` (signature per the ADR: the elapsed ticks
      ...
      `step_stp(&mut self, elapsed_ticks: u32) -> (u8, u8)`, in `crates/cortex-core/src/dynamics/plasticity.rs`; the returned pair is the input of `synaptic_efficacy_q16`.
- [x] Tests: a spike train at a fixed interval facilitates $u$ toward its fixed point and
      ...
      Six tests; the exponential factor has its own test since it is public.
- [x] Whitepaper §5.2.1 (public API, status), §8.8 row (the update becomes Implemented; the rest stays Specified), §8.1 if a
