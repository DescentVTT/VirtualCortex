...
- [ ] **The design, in a new ADR at the next free number (`ls docs/adr`), before the code.** It states:
...
- [ ] **The lane form in `cortex-core`**, beside `integrate` and unchanged from it in behaviour. It is held to
...
- [ ] **The executor's use of it**, if the design needs one. All of the tree's tests pass on it, the differential test
...
- [ ] **The per-turn breakdown after the change**: the one-worker turn at (a) beside `neuron/integrate`, and what now
  bounds a turn. This is what the next decision weighs.
- [ ] **The verdict in the round's ADR.**
