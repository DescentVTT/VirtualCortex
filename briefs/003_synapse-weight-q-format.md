...
- [ ] Decide Q8.8 or Q1.15 for `[i16; 4]` weights, with the argument written in a new ADR at the
      next free number (`ls docs/adr`; `status: proposed` in the PR, accepted on merge). The ADR
      must state how a weight is
      multiplied by the Q0.8 STP factors and accumulated into Q16.16 without loss of the intended
      bits.
- [ ] Rename the field to match (`weights_q8_8` or `weights_q1_15`); update its comment.
- [ ] Rename `intention_vector_ptr` to `intention_vector_idx`; update its comment.
- [ ] Add `pub const FORMAT_VERSION: u32` to `CortexFileHeader` (value 2; version 1 is the layout
      described by whitepaper 3.0.0) and cite it from the whitepaper §5.2.2 table.
- [ ] Whitepaper: §5.2.1 and §5.2.12 tables; §8.1 narrower-fields sentence; §12 glossary entry
      for the chosen format; F-3 and F-12 Resolved; the §11.1 open question checked off with the
      decision.
- [ ] `CHANGELOG.md` entry under Unreleased, including the format-version bump.
- [ ] Archive this brief.
